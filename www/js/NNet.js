...
function highlightNode(node, delay) {
    node.transition()
        // .delay(delay)
        .duration(500)
        .attr("r", 10) // Example: increase the radius
        .style("fill", "orange")
        .transition()
        .duration(500)
...
// Function to animate a link
function highlightLink(link, delay) {
    link.transition()
        .delay(delay)
        .duration(500)
        .style("stroke", "orange")
...
NNet.glow = function () {
    // Sequentially animate the neural network
...
console.log("nodes "+ nodes)
    ...
    nodes.each( function(d, i) {
        ...
        console.log("nodes "+ node+" "+delay)

        setTimeout(() => highlightNode(node, delay), delay);
        delay += 1000; // Delay between each node activation
    });

    links.each(function (d, i) {
        const link = d3.select(this);
        setTimeout(() => highlightLink(link, delay), delay);
        delay += 1000; // Delay between each link activation
    });
}